    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
        <Button variant="outline" size="sm" onClick={() => {
          ...
          onScan()
        }} disabled={loading} style={{ display: "flex", alignItems: "center", gap: "6px", opacity: dir.enabled ? 1 : 0.5 }}>
          <RefreshCw style={{ width: "16px", height: "16px", animation: loading ? "spin 1s linear infinite" : "none" }} />
          扫描
        </Button>
        <Button variant="outline" size="sm" onClick={onToggle} style={{ color: dir.enabled ? "#16a34a" : "#d97706" }}>
          {dir.enabled ? "启用" : "禁用"}
        </Button>
        <Button variant="ghost" size="icon" onClick={onDelete} style={{ color: "var(--color-destructive)" }}>
...
function TMDBApiKeySetting() {
  ...
  const handleSave = async () => {
    try {
      await import("@/lib/api").then(m => m.updateSetting("tmdb_api_key", apiKey || null))
      setSaved(true)
  ...
  return (
    <div>
      <div style={{ marginBottom: "12px" }}>
        <label style={{ fontSize: "14px", fontWeight: 500, color: "var(--color-foreground)", display: "block", marginBottom: "4px" }}>
          TMDB API Key
        </label>
        <p style={{ fontSize: "12px", color: "var(--color-muted-foreground)" }}>
          用于从 The Movie Database 获取影片信息。请到{" "}
          <a 
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          <div 
            style={{
              ...
              alignItems: "center",
              fontSize: "14px",
              color: apiKey ? "var(--color-foreground)" : "var(--color-muted-foreground)",
              fontFamily: "monospace"
            }}
          >
            {apiKey ? `${apiKey.substring(0, 20)}...` : "使用默认 API Key"}
          </div>
          <Button 
            variant="outline" 
            size="sm" 
